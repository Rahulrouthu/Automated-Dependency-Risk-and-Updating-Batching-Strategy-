...
import { Search, Filter, ArrowUpDown, ExternalLink, ShieldAlert, BookOpen, ChevronRight, Lock } from 'lucide-react';
  ...
  return (
    <div className="glass-panel rounded-2xl border border-slate-800 p-6 space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-bold text-white tracking-tight">Dependency Inventory &amp; Risk Matrix</h3>
          <p className="text-xs text-slate-400">
            Showing {filteredDependencies.length} of {dependencies.length} analyzed packages • Real lockfile &amp; security correlation
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-1.5 bg-slate-900 rounded-xl border border-slate-800 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-teal-500 font-mono w-44 sm:w-56"
            />
          </div>
          ...
          <select
            ...
            className="px-2.5 py-1.5 bg-slate-900 rounded-xl border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-teal-500 cursor-pointer"
          >
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="bg-slate-900/90 text-slate-400 font-semibold border-b border-slate-800 uppercase tracking-wider text-[10px]">
              ...
              <th className="py-3 px-3">Declared / Resolved</th>
              <th className="py-3 px-3">Target</th>
              <th className="py-3 px-3">Type</th>
              <th className="py-3 px-3">Security Priority</th>
              <th className="py-3 px-4">Composite Risk</th>
              <th className="py-3 px-4">Update Risk</th>
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <div className="font-semibold text-white font-mono group-hover:text-teal-400 transition">
                            {dep.name}
                          </div>
                          <div className="text-[11px] text-slate-400 font-mono flex items-center gap-1.5 mt-0.5">
                            {dep.groupOrNamespace ? `${dep.groupOrNamespace} • ` : ''}
                            <span className="text-slate-400">{dep.manifestFile}</span>
                            {dep.dev && <span className="px-1 rounded bg-slate-800 text-slate-400 text-[10px]">dev</span>}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-3">
                      ...
                      {hasVuln && (
                        <div className="text-[10px] text-rose-400 font-mono mt-0.5">
                          {dep.vulnerabilities.length} advisory ({dep.vulnerabilities[0].vulnId})
                        </div>
                      )}
                    </td>
                    ...
                    <td className="py-3 px-3 text-right">
                      <button className="p-1.5 rounded-lg bg-slate-800 text-slate-400 group-hover:text-teal-400 group-hover:bg-slate-700 transition">
